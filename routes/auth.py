...
def login():
    if (request.form.get('uname') and request.form.get('password')):
        user = db.session.execute(select(User).where(
            and_(User.uname == request.form.get('uname'), User.passwd == hashlib.md5(request.form.get('password').encode()).hexdigest()))).scalar()
        if user:
            login_user(user)
...
def register():
    if request.form:
        if string_check(request.form.get('uname')) and string_check(request.form.get('password')) and not db.session.execute(select(User).where(User.uname == request.form.get('uname'))).scalar():
            db.session.add(User(uname=request.form.get('uname'), passwd=str(hashlib.md5(request.form.get('password').encode()).hexdigest())))
            db.session.commit()
